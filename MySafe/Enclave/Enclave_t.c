...
typedef struct ms_get_file_istream_t {
	char* ms_path;
	uint8_t* ms_pointer;
	uint32_t* ms_size;
	uint32_t ms_offset;
} ms_get_file_istream_t;

typedef struct ms_get_file_ostream_t {
	char* ms_path;
	uint8_t* ms_pointer;
} ms_get_file_ostream_t;
...
static sgx_status_t SGX_CDECL sgx_enclave_encrypt_file(void* pms)
{
	...
	ms->ms_retval = enclave_encrypt_file(_in_path, _in_file_password, _tmp_len);
err:
	if (_in_path) free(_in_path);
...
static sgx_status_t SGX_CDECL sgx_enclave_decrypt_file(void* pms)
{
	...
	size_t _len_path = 100;
	char* _in_path = NULL;
	char* _tmp_newpath = ms->ms_newpath;
	size_t _len_newpath = 100;
	char* _in_newpath = NULL;
	char* _tmp_file_password = ms->ms_file_password;
	size_t _tmp_len = ms->ms_len;
	size_t _len_file_password = _tmp_len;
	char* _in_file_password = NULL;
	...
	CHECK_UNIQUE_POINTER(_tmp_path, _len_path);
	CHECK_UNIQUE_POINTER(_tmp_newpath, _len_newpath);
	CHECK_UNIQUE_POINTER(_tmp_file_password, _len_file_password);
	...
	if (_tmp_newpath != NULL) {
		_in_newpath = (char*)malloc(_len_newpath);
		if (_in_newpath == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memcpy(_in_newpath, _tmp_newpath, _len_newpath);
	}
	if (_tmp_file_password != NULL) {
		_in_file_password = (char*)malloc(_len_file_password);
		if (_in_file_password == NULL) {
	...
	ms->ms_retval = enclave_decrypt_file(_in_path, _in_newpath, _in_file_password, _tmp_len);
err:
	if (_in_path) free(_in_path);
	if (_in_newpath) free(_in_newpath);
	if (_in_file_password) free(_in_file_password);
...
sgx_status_t SGX_CDECL encalve_read_file(uint8_t* retval, char* path, char* buffer, size_t len, size_t* actual_len)
{
	...
	if (actual_len != NULL && sgx_is_within_enclave(actual_len, _len_actual_len)) {
		...
		memcpy(ms->ms_actual_len, actual_len, _len_actual_len);
...
sgx_status_t SGX_CDECL get_file_istream(char* path, uint8_t* pointer, uint32_t* size, uint32_t offset)
{
	sgx_status_t status = SGX_SUCCESS;
	size_t _len_path = 100;
	...
	ms_get_file_istream_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_get_file_istream_t);
	void *__tmp = NULL;
	...
	ms = (ms_get_file_istream_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_get_file_istream_t));
...
sgx_status_t SGX_CDECL get_file_ostream(char* path, uint8_t* pointer)
{
	sgx_status_t status = SGX_SUCCESS;
	size_t _len_path = 100;
	size_t _len_pointer = sizeof(*pointer);

	ms_get_file_ostream_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_get_file_ostream_t);
	void *__tmp = NULL;
	...
	ms = (ms_get_file_ostream_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_get_file_ostream_t));
