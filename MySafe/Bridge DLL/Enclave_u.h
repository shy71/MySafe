...
uint8_t SGX_UBRIDGE(SGX_NOCONVENTION, encalve_write_end_of_open_file, (char* path, char* buffer, size_t len, int call_type));
uint8_t SGX_UBRIDGE(SGX_NOCONVENTION, encalve_read_file, (char* path, char* buffer, size_t len, size_t* actual_len));
uint8_t SGX_UBRIDGE(SGX_NOCONVENTION, encalve_read_part_open_file, (char* path, char* buffer, size_t len, size_t* actual_len, int call_type));
